...
public class SimpleMask extends Transform<StructuredRecord, StructuredRecord> {
  ...
  private Map fieldsToScrambleMap;
  ...
  public void initialize(TransformContext context) throws Exception {
    ...
    fieldsToScrambleMap = new HashMap();

    String[] fields;
    if (config.scrambleText.contains(",")) {
      fields = config.scrambleText.split(",");
    } else {
       fields = new String[]{config.scrambleText};
    }

    for (String field : fields) {
      String[] kv = field.split(":");
      if (kv.length > 1) {
       fieldsToScrambleMap.put(kv[0], kv[1]);
      }
    }
  ...
  public void transform(StructuredRecord input, Emitter<StructuredRecord> emitter) throws Exception {
    ...
    for (Schema.Field field : input.getSchema().getFields()) {
      ...
      if (fieldsToScrambleMap.get(fieldName) != null) {
        outputValue = "*"+inputValue;
        //outputValue = Rot13.rotate(inputValue);
        //builder.set(fieldName, config.scrambleText);
      }
  ...
  public static class Config extends PluginConfig {
    ...
    public Config(String inputField, String schema) {
